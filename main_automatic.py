"""
AIREA Real Estate Chatbot API - Automatic Email Processing Version
AI-powered real estate assistant with automatic lead management and enhanced email logging
"""
...
def process_client_data(clients_list: List[Client], db: DatabaseManager, email_svc: MailjetEmailService, processing_method: str = "Automatic") -> tuple[int, List[str]]:
    """Process extracted client data: save to DB and send emails with enhanced logging."""
    processed_count = 0
    errors = []
    
    print(f"\n🚀 STARTING AUTOMATIC CLIENT DATA PROCESSING")
    ...
    for i, client_data in enumerate(clients_list, 1):
        try:
            ...
            # Use the enhanced email processing function for automatic sending
            if email_svc.is_configured():
                ...
                if email_result.get("success"):
                    print(f"✅ ALL EMAILS SENT AUTOMATICALLY for {client_data.name}")
                else:
...
async def root():
    """API root endpoint."""
    return {
        "message": "AIREA Real Estate Chatbot API - Automatic Email Processing",
        "version": Config.API_VERSION,
        "status": "operational",
        "features": {
...
async def chat_endpoint(
    ...
    """Main chat endpoint for conversing with the AI assistant with AUTOMATIC email processing."""
    try:
        ...
        # AUTOMATIC PROCESSING - Check if we should automatically extract data and send emails
        clients_processed = None
        if conv_manager.should_extract_data(conv_id):
            print(f"🚀 AUTOMATIC PROCESSING TRIGGERED for conversation {conv_id}")
...
async def auto_process_conversation(
    ...
    """Background task to automatically process conversation data with enhanced logging."""
    try:
        print(f"\n🤖 AUTOMATIC BACKGROUND PROCESSING STARTED")
        print(f"📋 Conversation ID: {conversation_id}")
        
        result = conv_manager.extract_and_process_clients(conversation_id)
        if result.get("success") and result.get("clients"):
            # Process clients with enhanced logging (save to DB and send emails)
            processed_count, errors = process_client_data(result["clients"], db, email_svc, "Automatic Background")
            ...
            print(f"⚠️  No clients extracted from conversation {conversation_id}")
    except Exception as e:
...
async def process_conversation_endpoint(
    ...
    try:
        ...
        processed_count, errors = process_client_data(
...
async def test_email(email_service: MailjetEmailService = Depends(get_email_service)):
    ...
    test_client = Client(
        ...
        property_type="House",
        budget=300000.0,
        details="This is a test email from the automatic AIREA system"
    )
...
if frontend_path.exists():
    ...
    async def serve_index():
        ...
        if index_path.exists():
            return FileResponse(index_path, media_type="text/html")
        return {"message": "AIREA Real Estate Chatbot API - Automatic Processing", "version": Config.API_VERSION, "note": "Frontend not found"}
...
if __name__ == "__main__":
    print("🚀 Starting AIREA Automatic Processing API Server...")
    print("📧 Email processing: AUTOMATIC")
